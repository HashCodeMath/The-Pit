...
public class setexpcommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
        if (s.hasPermission("commands.exp.set") || s.hasPermission("commands.exp.*") || s.hasPermission("commands.*")) {
            if (args.length == 0) {
                if (ThePit.getInstance().getConfig().getInt("Language") == 1) {
                    s.sendMessage(ThePit.messages.getString("Prefix") + ThePit.messages.getString("Russian.Commands.SetExpCommandUsage"));
                } else {
                    s.sendMessage(ThePit.messages.getString("Prefix") + ThePit.messages.getString("English.Commands.SetExpCommandUsage"));
                }
                return true;
            }
            ...
            if (args.length == 2) {
                ...
                if (parse(args[1])) {
                    ...
                    if (ThePit.getInstance().getConfig().getInt("Language") == 1) {
                        p.sendTitle(ThePit.messages.getString("Sender").replaceAll("<sender>", s.getName()), ThePit.messages.getString("Russian.Commands.SetedExp").replaceAll("<exp>", Integer.toString(i)));
                    } else {
                        p.sendTitle(ThePit.messages.getString("Sender").replaceAll("<sender>", s.getName()), ThePit.messages.getString("English.Commands.SetedExp").replaceAll("<exp>", Integer.toString(i)));
                    }
