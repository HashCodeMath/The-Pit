...
public class addlevelcommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
        if (s.hasPermission("commands.level.add") || s.hasPermission("commands.level.*") || s.hasPermission("commands.*")) {
            if (args.length == 0) {
                if (ThePit.getInstance().getConfig().getInt("Language") == 1) {
                    s.sendMessage(ThePit.messages.getString("Prefix") + ThePit.messages.getString("Russian.Commands.AddLevelCommandUsage"));
                } else {
                    s.sendMessage(ThePit.messages.getString("Prefix") + ThePit.messages.getString("English.Commands.AddLevelCommandUsage"));
                }
                return true;
            }
            ...
            if (args.length == 2) {
                ...
                if (parse(args[1])) {
                    ...
                    loadedUser.setLevel(loadedUser.getLevel() + i);

                    ListenerHelper.setupscoreboard(p);

                    if (ThePit.getInstance().getConfig().getInt("Language") == 1) {
                        p.sendTitle(ThePit.messages.getString("Sender").replaceAll("<sender>", s.getName()), ThePit.messages.getString("Russian.Commands.AddedLevel").replaceAll("<level>", Integer.toString(i)));
                    } else {
                        p.sendTitle(ThePit.messages.getString("Sender").replaceAll("<sender>", s.getName()), ThePit.messages.getString("English.Commands.AddedLevel").replaceAll("<level>", Integer.toString(i)));
                    }
